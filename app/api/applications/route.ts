...
export async function GET(req: NextRequest) {
    ...
    try {
        const appResult = await pool.query(
            statusFilter
                ? `SELECT * FROM applications WHERE current_status = $1 ORDER BY last_updated_at DESC`
                : `SELECT * FROM applications ORDER BY last_updated_at DESC`,
            statusFilter ? [statusFilter] : []
        );

        const applications: Application[] = appResult.rows.map((row) => ({
            ...row,
            events: [],
        }));
        ...
        const funnelResult = await pool.query(`
            SELECT
                COUNT(DISTINCT id)::int AS applied,
                COUNT(DISTINCT CASE WHEN ever_interviewed THEN id END)::int AS interview,
                COUNT(DISTINCT CASE WHEN current_status = 'offer' THEN id END)::int AS offer
            FROM (
                     SELECT a.id, a.current_status,
                            EXISTS (
        ...
        return NextResponse.json({ applications, summary, funnel }, { status: 200 });
    } catch (err) {
